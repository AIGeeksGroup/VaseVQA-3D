...
# -*- coding: utf-8 -*-
"""
多视角图片描述生成器 - InternVL版本
基于InternVL本地模型，处理6个面的图片生成综合caption
"""
...
def generate_multiview_caption(model_dir: str, pipe):
    """为单个模型的多视角图片生成综合描述"""
    ...
    # 加载所有图片
    image_paths = list(view_images.values())
...
"""
    ...
    try:
        print(f"\n正在调用 InternVL 进行多视角分析...")

        # 调用模型
        response = pipe((prompt_text, images))
...
def batch_generate_captions(input_dir: str, output_dir: str):
    """批量处理多个模型的多视角图片"""

    print(f"多视角图片描述生成器 - InternVL版本")
    ...
    # 如果你想用多卡，就把 tp=1 改成 tp=N（N为卡数），并提前 export CUDA_VISIBLE_DEVICES=0,1,2,...
    ...
    # 生成输出文件名：image_internvl.json
    output_filename = "image_internvl.json"
    ...
    for i, model_dir_name in enumerate(model_dirs):
        ...
        try:
            # 生成描述
            caption = generate_multiview_caption(model_dir_path, pipe)
    ...
    report_file = os.path.join(output_dir, "internvl_caption_generation_report.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='多视角图片描述生成器 - InternVL版本')
    ...
    parser.add_argument('--model_path', type=str,
                       default='./models/OpenGVLab/InternVL3_5-4B',
                       help='InternVL模型路径')
    ...
    if args.single_model:
        ...
        caption = generate_multiview_caption(args.single_model, pipe)
        ...
        # 批量处理
        batch_generate_captions(args.input_dir, args.output_dir)

if __name__ == '__main__':
    mp.set_start_method('spawn', force=True)   # 可选，强制统一 spawn
    main()